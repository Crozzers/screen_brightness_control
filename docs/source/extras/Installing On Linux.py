...
The library can call upon external programs if the native methods fail.
...
Program       | Works on laptop displays | Works on external monitors | Requires Special Permissions After Install
--------------|--------------------------|----------------------------|-------------------------------------------------------------------
ddcutil       | No                       | Yes (slowest) [1]          | Read/write access for `/dev/i2c*` (see [above](#desktop-displays))
[No program]  | Yes                      | Yes (slow)                 | See [above](#without-using-a-3rd-party-program)
